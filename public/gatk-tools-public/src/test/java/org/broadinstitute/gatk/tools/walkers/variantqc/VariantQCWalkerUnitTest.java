...
public class VariantQCWalkerUnitTest extends BaseTest {
    VariantQC walker;
    VariantContext eval;

    private class VariantQCTestProvider extends TestDataProvider {
        final List<Integer> expectedCounts;
        final int maxI;

        public VariantQCTestProvider(int maxI, final List<Integer> expectedCounts, final List<Integer> ... allStates) {
            super(VariantQCTestProvider.class);
    ...
    @BeforeMethod
    public void init() {
        walker = new VariantQC();
        eval = new VariantContextBuilder("x", "chr1", 1, 1, Collections.singleton(Allele.create("A", true))).make();
    }

    @Test(dataProvider = "VariantQCTestProvider")
    public void testBasicOperation(VariantQCTestProvider cfg) {
        //initialize(cfg);
